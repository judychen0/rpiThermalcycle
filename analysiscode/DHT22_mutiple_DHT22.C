...
void DHT22_mutiple_DHT22()
{
  std::ifstream file("./textfile/DHT22_.txt", std::ifstream::in);
  TString line;
  TGraph *g = new TGraph();   // temperature graph 
  TGraph *g1 = new TGraph();  // humidity graph
  TGraph *g2 = new TGraph();  // dewpoint graph
  TGraph *g3 = new TGraph();  // temp - dewpoint graph
  TGraph *l5 = new TGraph();  //
  TGraph *l6 = new TGraph();  //
  TGraph *l7 = new TGraph();  // max avg temperature 
  TGraph *l8 = new TGraph();  // min avg temperature
  

  TString ymd, hms;                        //readin datetime
  Float_t temp, humid, dewpoint;           //DHT22 data
  TString modeNo, stepNo, setTime, state;  //readin hitachi data(not important)
  Int_t lapse;                             //lapse time from first recording time
  Int_t rep, restep, sig, hold;            //readin hitachi data(not important)
  Float_t setTemp, setHumid;               //readin hitachi temperature and humidity
  Float_t delta_temp;                      //temp - setTemp
  Int_t hr, min, sec;                      //convert lapse seconds to hr, min, sec
  
  while(file >> ymd >> hms >> lapse >> temp >> humid >> dewpoint >> modeNo >> stepNo >> setTemp >> rep >> setTime >> setHumid >> restep >> sig >> hold >> state)
    {
      //std::cout << "col1 : " << ymd << " col2 : " << hms << std::endl;
      //TDatime *t = new TDatime(ymd +" "+ hms);

      // convert lapse seconds
      hr = lapse / 3600;
      min = (lapse % 3600) / 60;
      sec = (lapse % 3600) % 60;
  ...
  // set cursor line at 40*C hold 0:00
  ...
  */
  ...
  // set cursor line at 40*C hold 0:10
  ...
  */
  ...
  // set cursor line at -10*C hold 0:00
  ...
  */
  ...
  // create multigraph and add graphs to draw superimposing graph
